...
export const TodoList = (props: TodoListProps) => {
  ...
  const handleCheckTodo = (id: number, checked: boolean) => {
    updateStatus({
      todoId: id,
  ...
  return (
    <>
      <ul className="flex flex-col gap-y-3" ref={animationParent}>
        {todos
          .filter((item) => props.valueStatus.includes(item.status))
          .map((todo) => (
            <li key={todo.id}>
              <div
